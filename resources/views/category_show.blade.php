                ...
                @if($node->getDescendantCount()>0)
                    <ul class="sub-menu-ul">
                        @foreach($node->getDescendants() as $descend)
                            <li><a href="{{URL::to('/'.$descend->slug)}}">{{$descend->name}}</a></li> <!--ссылки на подкатегории -->
                        @endforeach
                    </ul>
                @endif
...
@else
    ...
    @if($node->getDescendantCount()>0)
        <ul class="sub-menu-ul">
            @foreach($node->getDescendants() as $descend)
                <li><a href="{{URL::to('/'.$descend->slug)}}">{{$descend->name}}</a></li>
            @endforeach
            <li><a href="{{URL::to('/'.$node->slug)}}">Назад</a></li>
        </ul>
    @endif
    ...
    @foreach($node->getAncestors() as $descend)
        <li class="ibl">-><a href="{{URL::to('/'.$descend->slug)}}">{{$descend->name}}</a></li>
    @endforeach
